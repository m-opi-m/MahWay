...
// ============================================================================
// التوصيل المباشر للعميل (تخطي نقطة التوزيع)
// ============================================================================
// لو مندوب الشحن (وهو حائز الشحنة في مرحلة DIRECT_DELIVERY_FROM_STATUS) قرر
// إنه يوصّل الشحنة للعميل بنفسه من غير ما يعديها على نقطة التوزيع، بيقدر
// يستخدم زرار "توصيل مباشر للعميل" (جنب الزرار العادي بتاع المرحلة الطبيعية
// اللي بعدها) بدل ما يستنى المسار العادي. الزرار ده بيولّد كود تسليم لـ
// DIRECT_DELIVERY_TO_STATUS على طول (نفس آلية الكود العادية بالظبط)، فلما
// الكود ده يتأكد، الشحنة بتبقى "في يد الموزع" - وعندها صفحة تتبع العميل
// العامة بتعرض اسم/تليفون/صورة الموزع (نفس بيانات الموظف اللي أكّد الكود)،
// وآخر خطوة تفضل زي العادي: الموزع يولّد كود "تم التسليم للعميل" والعميل
// بس اللي يدخله في صفحة التتبع عشان يتأكد الاستلام فعليًا.
const DIRECT_DELIVERY_FROM_STATUS = 'to_distribution_point';
const DIRECT_DELIVERY_TO_STATUS = 'with_distributor';
...
function doGet(e) {
  try {
    ...
    if (action === 'orderDetails') {
      return jsonResponse(getOrderDetailsForLabel(e.parameter.token, e.parameter.code));
    }
...
function handleOrderSubmit(data) {
  ...
  sendBrandedEmail(NOTIFY_EMAIL, subject, body);
...
// إرسال إيميل "علامة تجارية" موحّد لكل رسائل النظام: بيضيف اسم مرسل واضح
// (MahWay بدل الإيميل الخام)، وreply-to لإيميل الإدارة، وتوقيع/فوتر تحت كل
// رسالة. الهدف إن الرسائل تبان متكاملة ومصدرها واضح، بدل ما تكون نص قصير
// عاري بيشبه رسائل السبام الأوتوماتيك، وده بيقلل احتمال إن Gmail يصنّفها Spam
function sendBrandedEmail(to, subject, plainBody, options) {
  const opts = options || {};
  const htmlBody =
    '<div dir="rtl" style="font-family:Tahoma,Arial,sans-serif;line-height:1.9;color:#1f2937;font-size:15px;">' +
    plainBody.replace(/\n/g, '<br>') +
    '<hr style="margin:28px 0 16px;border:none;border-top:1px solid #e5e7eb;">' +
    '<p style="font-size:12.5px;color:#6b7280;margin:0;">' +
    '<strong style="color:#374151;">MahWay</strong> — Import • Export • Shipping<br>' +
    'لأي استفسار راسلنا على ' + NOTIFY_EMAIL + '<br>' +
    'دي رسالة تلقائية من نظام الطلبات.' +
    '</p></div>';
...
function sendTrackingCodeEmail(customerEmail, customerName, trackingCode) {
  ...
  try {
    sendBrandedEmail(customerEmail, subject, body);
  } catch (err) {
    // لو حصل خطأ في إرسال الإيميل للعميل، ميوقفش باقي العملية - لكن نبعت
    // تنبيه لإيميل الإدارة بنص الخطأ بالظبط عشان يبان السبب فورًا (Quota
    // متخطاة، Authorization ناقصة، ...إلخ) من غير ما حد يحتاج يدخل يدوّر
    // في سجل الـ Executions جوا Apps Script
    console.error('فشل إرسال إيميل كود التتبع للعميل: ' + err.toString());
    try {
      MailApp.sendEmail(
        NOTIFY_EMAIL,
        '⚠️ فشل إرسال كود التتبع لعميل - MahWay',
        'حصل خطأ وإحنا بنحاول نبعت كود التتبع للعميل على إيميله.\n\n' +
        'إيميل العميل: ' + customerEmail + '\n' +
        'اسم العميل: ' + (customerName || '-') + '\n' +
        'كود التتبع: ' + trackingCode + '\n\n' +
        'نص الخطأ: ' + err.toString()
      );
    } catch (notifyErr) {
      console.error('فشل حتى إرسال تنبيه الخطأ للإدارة: ' + notifyErr.toString());
    }
  }
}
...
// 19 UpdatedByPhone (هاتف آخر تحديث)
// 20 UpdatedByPhotoUrl (صورة آخر تحديث - بتتعرض للعميل في صفحة التتبع
//    العامة وقت ما الشحنة تبقى "في يد الموزع"، جنب اسمه وتليفونه)
const SHIPMENTS_HEADERS = [
  ...
  'المحافظة', 'هاتف آخر تحديث', 'صورة آخر تحديث'
];
...
function getShipmentsSheet() {
  ...
  // بنخزّن الأعمدة دي (تاريخ آخر تحديث / تاريخ الإنشاء) كـ epoch ms (رقم عادي)
  // مش كـ Date، عشان تتقرأ بنفس القيمة أيًا كان تايم زون المشروع. لو العمود
  // ده اتفورمات قبل كده كـ "تاريخ/وقت" (يدويًا أو تلقائيًا من جوجل شيتس)،
  // جوجل شيتس بتفسّر أي رقم بيتكتب فيه كـ "serial date number" مش كـ epoch ms،
  // وده بيطلع تاريخ فاسد (خارج مدى JS) وبيكسر new Date(...).toISOString()
  // بـ RangeError: Invalid time value. بنجبر الفورمات هنا كل مرة عشان نضمن
  // إن ده مايتكررش تاني حتى لو حد فورمات العمود يدوي بعد كده.
  enforcePlainNumberFormat(sheet);
  return sheet;
}

function enforcePlainNumberFormat(sheet) {
  const numRows = Math.max(sheet.getMaxRows() - 1, 1);
  sheet.getRange(2, 7, numRows, 1).setNumberFormat('0'); // تاريخ آخر تحديث
  sheet.getRange(2, 8, numRows, 1).setNumberFormat('0'); // تاريخ الإنشاء
}

// دالة تشغيل يدوية لمرة واحدة (شغّلها من محرر الأبس سكريبت: اختارها من
// القائمة المنسدلة فوق واضغط Run) عشان تصلّح أي صفوف اتخزّنت غلط قبل الفيكس
// ده - يعني أي شحنة أصلاً بتاريخها ظاهر كـ Date فاسد بسبب فورمات العمود القديم.
function repairShipmentDateColumns() {
  const sheet = getShipmentsSheet(); // ده هيظبط الفورمات كمان
  const lastRow = sheet.getLastRow();
  if (lastRow < 2) {
    Logger.log('مفيش شحنات لإصلاحها');
    return;
  }

  const range = sheet.getRange(2, 7, lastRow - 1, 2); // عمودي G و H
  ...
  for (let i = 0; i < values.length; i++) {
    for (let c = 0; c < 2; c++) {
      ...
      if (v instanceof Date) {
        // ده الفورمات القديم الفاسد - القيمة الأصلية اتفقدت (اتفسّرت غلط)، فبنحطّ
        // وقت دلوقتي كبديل آمن عشان الشحنة ما تفضلش عالقة بتاريخ فاسد يكسر الداشبورد
        const t = v.getTime();
        values[i][c] = isNaN(t) ? now : t;
        fixedCount++;
      }
    }
  }
  ...
  Logger.log('تم إصلاح ' + fixedCount + ' خلية تاريخ فاسدة من أصل ' + (values.length * 2));
}

// بديل آمن لـ new Date(value).toISOString() - بيرجّع '' بدل ما يرمي
// RangeError لو القيمة مش قابلة للتحويل لتاريخ صالح (بدل ما يوقع الفانكشن
// كلها ويرجّع خطأ لكل الشحنات بسبب صف واحد بس فيه بيانات فاسدة)
...
function trackShipment(code) {
  ...
  return {
    ...
    // اسم/صورة الموزع - بيتعرضوا في صفحة التتبع العامة بس وقت ما الحالة تبقى
    // "with_distributor" (الفرونت هو اللي بيتحكم في العرض حسب statusKey،
    // بالظبط زي distributorPhone فوق)
    distributorName: values[5] || '',
    distributorPhotoUrl: values[19] || '',
    lastUpdate: safeDateToISO(values[6]),
    createdAt: safeDateToISO(values[7]),
    latitude: values[8] || '',
    longitude: values[9] || '',
...
// ---------- تفاصيل الطلب لطباعة بوليصة الشحن ----------
// بترجع تفاصيل الطلب (الوزن/نوع الشحن/المسار/الملاحظات...) من شيت Orders عشان
// تتطبع على بوليصة الشحن اللي بتتحط على الشحنة فعليًا. أي موظف عنده جلسة
// شغالة يقدر يستخدمها (مش مقصورة على دور معيّن) لأنها مجرد بيانات طباعة.
// ملحوظة أمان مهمة: الفانكشن دي عمدًا مابترجعش كود التتبع الأصلي بتاع
// العميل في أي مكان في الرد - الكود ده لازم يفضل سري وميتطبعش على بوليصة
// ورقية ممكن تتفتح لأي حد.
function getOrderDetailsForLabel(token, trackingCode) {
  const session = getSessionByToken(token);
  if (!session) {
    return { result: 'error', message: 'الجلسة منتهية. سجل الدخول تاني.' };
  }

  const code = (trackingCode || '').toString().trim().toUpperCase();
  if (!code) {
    return { result: 'error', message: 'كود غير صالح' };
  }

  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  const sheet = ss.getSheetByName(ORDERS_SHEET_NAME);
  if (!sheet) {
    return { result: 'error', message: 'شيت الطلبات مش موجود' };
  }

  const lastRow = sheet.getLastRow();
  if (lastRow < 2) {
    return { result: 'error', message: 'مفيش بيانات طلب لهذه الشحنة' };
  }

  const rows = sheet.getRange(2, 1, lastRow - 1, 13).getValues();
  for (let i = 0; i < rows.length; i++) {
    const rowCode = (rows[i][9] || '').toString().trim().toUpperCase();
    if (rowCode === code) {
      return {
        result: 'success',
        orderDate: safeDateToISO(rows[i][0]),
        weight: rows[i][4] || '',
        shippingType: rows[i][6] || '',
        shippingRoute: rows[i][7] || '',
        notes: rows[i][8] || '',
        governorate: rows[i][12] || ''
        // عمدًا مفيش أي حقل بيرجّع كود التتبع (rows[i][9]) هنا
      };
    }
  }

  return { result: 'error', message: 'مفيش بيانات طلب إضافية لهذه الشحنة' };
}
...
function getEmployeeByEmail(email) {
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (rowEmail === target) {
      return { name: rows[i][2] || '', phone: rows[i][4] || '', photoUrl: rows[i][6] || '' };
    }
  }
  return null;
}
...
function getShipmentsForEmployee(token) {
  ...
  if (lastRow >= 2) {
    ...
    rows.forEach(row => {
      ...
      // زرار "توصيل مباشر للعميل" (تخطي نقطة التوزيع): متاح بس لو الشحنة في
      // مرحلة DIRECT_DELIVERY_FROM_STATUS وحائزها فعلاً هو الموظف ده، ومفيش
      // كود معلّق عليها حاليًا - بالظبط نفس شروط الزرار العادي (canGenerateNext)
      // لكن بيولّد كود لـ DIRECT_DELIVERY_TO_STATUS على طول بدل المرحلة اللي بعدها مباشرة
      const canDirectDeliver = !pendingActive && ownedByMe
        && statusKey === DIRECT_DELIVERY_FROM_STATUS
        && !!SHIPMENT_STATUSES[DIRECT_DELIVERY_TO_STATUS];
      ...
      shipments.push({
        ...
        lastUpdate: safeDateToISO(row[6]),
        createdAt: safeDateToISO(row[7]),
        latitude: row[8] || '',
        longitude: row[9] || '',
        ...
        canDirectDeliver: canDirectDeliver,
        directDeliverToStatus: canDirectDeliver ? DIRECT_DELIVERY_TO_STATUS : '',
        directDeliverLabel: canDirectDeliver ? SHIPMENT_STATUSES[DIRECT_DELIVERY_TO_STATUS].ar : '',
...
function handleGenerateHandoffCode(data) {
  ...
  // زرار "توصيل مباشر للعميل": بدل ما ياخد الشحنة على المسار الطبيعي
  // (المرحلة اللي بعده مباشرة)، الموظف ممكن يطلب صراحة إنه يتخطى نقطة
  // التوزيع ويوديها على طول لـ DIRECT_DELIVERY_TO_STATUS - متاح بس من
  // DIRECT_DELIVERY_FROM_STATUS، وبنفس آلية الكود العادية تمامًا بعد كده
  const wantsDirectDelivery = !!data.directToDistributor;
  let nextStatus;
  if (wantsDirectDelivery) {
    if (currentStatus !== DIRECT_DELIVERY_FROM_STATUS) {
      return { result: 'error', message: 'التوصيل المباشر متاح بس والشحنة في الطريق لنقطة التوزيع' };
    }
    if (!SHIPMENT_STATUSES[DIRECT_DELIVERY_TO_STATUS]) {
      return { result: 'error', message: 'مرحلة التوصيل المباشر غير متاحة حاليًا' };
    }
    nextStatus = DIRECT_DELIVERY_TO_STATUS;
  } else {
    nextStatus = getEffectiveNextStatusKey(currentStatus, governorate);
  }
  ...
  logAudit({
    ...
    notes: wantsDirectDelivery
      ? 'تم توليد كود توصيل مباشر للعميل (تخطي نقطة التوزيع)'
      : 'تم توليد كود تسليم/استلام للمرحلة الجاية'
  });
  ...
  return {
    ...
    directDelivery: wantsDirectDelivery,
...
function handleConfirmHandoffCode(data) {
  ...
  // هاتف/صورة الحائز الجديد فعليًا للشحنة (بيتعرضوا في بوكس "الشحنة حاليًا
  // مع" في البوابة، وفي بوكس الموزع بصفحة تتبع العميل العامة وقت ما تبقى
  // الحالة "with_distributor")
  sheet.getRange(row, 19).setValue(session.phone || '');
  sheet.getRange(row, 20).setValue(session.photoUrl || '');
...
function handleConfirmDelivery(data) {
  ...
  if (isPickupStage) {
    ...
    const holderPhotoUrl = (generatorEmployee && generatorEmployee.photoUrl) || '';
    sheet.getRange(row, 6).setValue(holderName);
    sheet.getRange(row, 19).setValue(holderPhone);
    sheet.getRange(row, 20).setValue(holderPhotoUrl);
  } else {
