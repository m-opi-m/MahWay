...
const translations = {
    ar: {
        ...
        "tracking.withDistributorTitle": "شحنتك حاليًا مع الموزع",
        ...
        "form.addressPlaceholder": "مثال: القاهرة - مدينة نصر - شارع مكرم عبيد - عمارة 15 - أمام صيدلية النور - الدور الثالث - شقة 7",
        ...
        "service.feature.countries": "🌍 8 دول",
    ...
    en: {
        ...
        "tracking.withDistributorTitle": "Your shipment is currently with the distributor",
        ...
        "form.addressPlaceholder": "Example: Cairo - Nasr City - Makram Ebeid St. - Building 15 - opposite Al Nour Pharmacy - 3rd floor - Apt 7",
        ...
        "service.feature.countries": "🌍 8 Countries",
    ...
    tr: {
        ...
        "tracking.withDistributorTitle": "Gönderiniz şu anda dağıtıcıda",
        ...
        "form.address": "Detaylı Adres *",
        "form.addressPlaceholder": "Örnek: Kahire - Nasr City - Makram Ebeid Cad. - Bina 15 - Al Nour Eczanesi karşısı - 3. kat - Daire 7",
        ...
        "service.feature.countries": "🌍 8 Ülke",
...
document.querySelectorAll('.lang-btn').forEach(btn => {
    btn.addEventListener('click', function() {
        ...
        switchLanguage(lang);
        if (langCurrent) {
            langCurrent.textContent = this.textContent;
        }
        if (langDropdown) {
            langDropdown.classList.remove('open');
        }
...
function fixTouchIssues() {
    ...
    // تحسين أداء التمرير - نمنع الزوم بإصبعين بس (pinch)، من غير ما نأثر على السكرول العادي
...
(function initGovernorateToggle() {
    ...
    function syncGovernorateVisibility() {
        if (routeSelect.value === 'egypt') {
            ...
            const trigger = document.querySelector('#governorateCustomSelect .custom-select-trigger');
            const valueText = document.querySelector('#governorateCustomSelect .custom-select-value');
            if (trigger && valueText) {
                valueText.textContent = (translations[currentLanguage] && translations[currentLanguage]['form.chooseGovernorate']) || 'اختر المحافظة';
                valueText.setAttribute('data-i18n', 'form.chooseGovernorate');
                document.querySelectorAll('#governoratePanel .custom-select-option.active').forEach(function (o) {
    ...
    // أول ما العميل يختار المحافظة، ننقل الخريطة عليها تلقائيًا (وهو لسه يقدر يسحب الدبوس بعد كده لضبط المكان بالظبط)
    governorateSelect.addEventListener('change', function () {
        const coords = GOVERNORATE_COORDS[governorateSelect.value];
        if (coords && typeof window.__moveOrderMapTo === 'function') {
            window.__moveOrderMapTo(coords[0], coords[1], 11);
        }
    });
})();
...
function renderTrackingResult(data) {
    ...
    if (data.status === 'with_distributor' && data.distributorPhone) {
        ...
        const nameEl = document.getElementById('distributorNameValue');
        if (nameEl) nameEl.textContent = data.distributorName || '';

        const avatarEl = document.getElementById('distributorAvatar');
        const fallbackEl = document.getElementById('distributorAvatarFallback');
        if (avatarEl && fallbackEl) {
            if (data.distributorPhotoUrl) {
                avatarEl.src = data.distributorPhotoUrl;
                avatarEl.style.display = 'block';
                fallbackEl.style.display = 'none';
                // لو رابط الصورة اتكسر لأي سبب، نرجع لأيقونة الفولباك تلقائيًا
                avatarEl.onerror = () => {
...
// إحداثيات تقريبية لمركز كل محافظة (لتحريك الخريطة عليها أول ما العميل يختارها)
const GOVERNORATE_COORDS = {
    'القاهرة': [30.0444, 31.2357],
    'الجيزة': [30.0131, 31.2089],
    'القليوبية': [30.1792, 31.2044],
    ...
    'شمال سيناء': [31.1315, 33.7994],
    'جنوب سيناء': [28.2298, 33.6198],
    ...
    'البحر الأحمر': [26.0975, 33.8250],
    'الوادي الجديد': [25.4478, 30.5486]
};
...
function initOrderMapPicker() {
    ...
    // إتاحة تحريك الخريطة من بره الدالة دي (مثلاً أول ما تتحدد المحافظة)
    window.__moveOrderMapTo = function (lat, lng, zoom) {
        placeAt(lat, lng, zoom);
    };
...
// قوائم منسدلة مخصصة (زي المحافظة) - بتفتح لتحت دايمًا وبشكل منظم
function initCustomSelects() {
    document.querySelectorAll('.custom-select').forEach(function (wrapper) {
        ...
        const valueText = wrapper.querySelector('.custom-select-value');
        const hiddenSelect = document.getElementById(wrapper.getAttribute('data-target'));
        ...
        panel.querySelectorAll('.custom-select-option').forEach(function (option) {
            option.addEventListener('click', function () {
                const val = option.getAttribute('data-value');
                hiddenSelect.value = val;
                valueText.textContent = option.textContent.trim();
                valueText.removeAttribute('data-i18n');
...
// منع تكبير الصفحة بإصبعين على iOS، من غير ما نأثر على سكرول اللمسة الواحدة
document.addEventListener('touchmove', function (event) {
